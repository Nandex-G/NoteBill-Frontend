  ...
  return (
    <div className='absolute top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2 bg-gray-500 rounded-2xl'>
        ...
        <div className='flex flex-col justify-center items-center p-5 sm:p-10'>
          <span className='text-white text-lg md:text-3xl mb-3 pb-3 md:mb-6 md:pb-6 w-full text-center border-b-2'>ثبت‌نام</span>
          <form className='mb-6 sm:mb-12 flex flex-col gap-5 sm:gap-10 text-xs md:min-w-96' onSubmit={handleSubmit} >
            <input className='bg-white rounded-2xl outline-0 p-2 sm:py-2 sm:px-5 text-center text-sm sm:text-xl' type="text" name="username" placeholder="نام کاربری" value={form.username} onChange={handleChange} required/>
            <input className='bg-white rounded-2xl outline-0 p-2 sm:py-2 sm:px-5 text-center text-sm sm:text-xl' type="text" name="password" placeholder="رمز عبور" value={form.password} onChange={handleChange} required/>
            <button className='w-full bg-green-500 rounded-2xl p-2 hover:bg-green-600 sm:text-lg duration-200' type="submit">ثبت‌نام</button>
          </form>
          <div className='text-xs sm:text-sm'>حساب کاربری دارید؟<Link to={'/login'} className='underline cursor-pointer text-blue-800 hover:text-blue-700 duration-150'> وارد شوید </Link></div>
        </div>
        {/* Loading */}
        <div className={`${loadingDisplay} ${loadingOpacity} none rounded-2xl absolute z-50 top-0 w-full h-full bg-black/30 backdrop-blur-sm justify-center items-center duration-200 `}>
