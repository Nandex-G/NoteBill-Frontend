...
function BillCard({ title, name, fullPrize, paidPrize, id }) {
  ...
  const [fullPrizeState, setFullPrizeState] = useState(fullPrize)
  ...
  function submitEdit(id, e) {    
    e.preventDefault()
    if (englishTextTransformer(e.currentTarget.querySelectorAll('input')[2].value) <= paidPrize) {
    ...
    <div className="min-w-xs sm:min-w-sm lg:mx-5 mb-8">
      <div className="bg-white rounded-2xl shadow-xl md:py-8 mx-4 md:m-0 py-4 px-4 relative flex flex-col justify-center items-center">
        ...
        <div className="mt-5 border-2 bg-white md:text-lg text-sm rounded-xl text-center border-indigo-500 w-full text-[#111827] md:p-4 sm:px-5 py-2 flex justify-between">
          ...
          <span className="sm:w-1/3 w-1/2 duration-200">{Number.parseInt(fullPrize).toLocaleString('fa-IR')}</span>
        </div>
        <div className="flex w-full justify-between sm:px-5 mt-6">
          <div className="text-center w-1/2 mx-2">
            ...
            <span className="mt-2 block text-[#991B1B] text-xs sm:text-sm md:text-lg bg-red-300 rounded-2xl sm:px-5 px-1 sm:py-7 py-2 duration-200 text-center">{(fullPrize - paidPrize).toLocaleString('fa-IR')}</span>
          </div>
          <div className="text-center w-1/2 mx-2">
            <h3 className="md:text-[16px] sm:text-sm text-xs font-bold duration-200 text-nowrap select-none">پرداخت شده :</h3>
            <span className="mt-2 block text-[#065F46] text-xs sm:text-sm md:text-lg bg-green-200 rounded-2xl sm:px-5 px-1 sm:py-7 py-2 duration-200 text-center">{paidPrize.toLocaleString('fa-IR')}</span>
          </div>
        </div>
        <Button onClick={() => showPayment() } bgColor ={paidPrize == fullPrize ? 'bg-gray-300' : 'bg-orange-400'} hoverBgColor ={paidPrize == fullPrize ? 'hover:bg-gray-400' : 'hover:bg-orange-500'} value='افزایش پرداخت شده' classNames="select-none rounded-lg sm:mt-10 mt-5 border transition-all py-2 sm:w-1/2 w-2/3 shadow-md hover:shadow-lg text-xs sm:text-lg"/>
        ...
        <form onSubmit={(e) => submitEdit(id, e)} className={`${editDisplay} ${editOpacity} duration-200 absolute w-full h-full rounded-2xl bg-emerald-100 z-10 top-0 flex-col items-center justify-center`}>
          ...
          <Input placeHolder='مبلغ' value={persianTextTransformerWithoutEvent(String(fullPrizeState))} type='text' onInput={(e) => {setFullPrizeState(e.currentTarget.value); setEditMessage('مبلغ کل بدهی'); setEditInputLabelColor('text-gray-700 md:text-lg lg:text-xl')}} parentClassNames='text-center w-full flex flex-col items-center py-1 sm:py-5 px-2' classNames='text-center bg-white shadow-inner outline-none w-full sm:w-5/6 py-2 mt-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-emerald-400 EditFullPrize' labelColor={editInputLabelColor} label={editMessage} required />
